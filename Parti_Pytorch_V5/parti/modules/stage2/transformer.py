...
class Transformer(nn.Module):
    def __init__(self, text_vocab_size: int , codebook_vocab_size: int, dim_model: int, nhead: int, num_encoder_layers: int, num_decoder_layers: int, dim_feedforward: int, dropout: float):
        super().__init__()
        self.positional_encoding=PositionalEncoding(dim_model)
        self.encoder_embedding=nn.Embedding(text_vocab_size, dim_model)
        self.decoder_embedding=nn.Embedding(codebook_vocab_size, dim_model)
        self.transformer=BaseTransformer(dim_model, nhead, num_encoder_layers, num_decoder_layers, dim_feedforward, dropout)


    def make_tgt_mask(self,tgt: torch.IntTensor)->torch.IntTensor:
        """
        Args:
            trg: target sequence
        Returns:
    ...
    def forward(self,x_images_token: torch.IntTensor, x_texts_token: torch.IntTensor)->torch.FloatTensor:
        ...
        x_images_mask=self.make_tgt_mask(x_images_token)
